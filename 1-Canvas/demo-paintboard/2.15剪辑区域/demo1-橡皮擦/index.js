/***
 * 橡皮擦：
 *      1. 将当前canvas的剪辑区域设置为  橡皮擦所在区域
 *      2. clearRect()
 * action:
 *      mousemove：擦除上一次橡皮擦所在区域 & 在当前位置绘制橡皮擦
 *      clip(): 将会把剪辑区域设置为当前剪辑区域与当前路径的交集
 *              一般嵌入在save() 与 restore() 之间使用
 * 
 */
...
function setDrawPathForEraser(loc) {
   ...
   context.clip(); // 剪辑区域 =  当前剪辑区域 与 当前路径 交集
}
